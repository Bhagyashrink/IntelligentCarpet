...
parser.add_argument('--exp_dir', type=str, default='./IntelligentCarpet-main/', help='Experiment path')
...
parser.add_argument('--epoch', type=int, default=25, help='The time steps you want to subsample the dataset to,500')
parser.add_argument('--ckpt', type=str, default ='retrain_original_model_pushUp_test_dataset_0.0001_10_best', help='loaded ckpt file')
parser.add_argument('--eval', type=bool, default=True, help='Set true if eval time')
parser.add_argument('--test_dir', type=str, default ='/IntelligentCarpet-main/singlePerson_test_diffTask/pushUp/', help='test data path')
...
if args.linkLoss:
    link_min = pickle.load(open(args.exp_dir + 'train/link_min.p', "rb"))
    link_max = pickle.load(open(args.exp_dir + 'train/link_max.p', "rb"))
...
#loading pushUp test dataset
test_dataset = "/IntelligentCarpet-main/singlePerson_test_diffTask/pushUp/"
t_dataset = sample_data_diffTask(test_dataset, args.window, args.subsample)
train_dataloader = DataLoader(t_dataset, batch_size=args.batch_size,shuffle=True, num_workers=8)
val_dataloader = DataLoader(t_dataset, batch_size=args.batch_size, shuffle=False, num_workers=8)
test_dataloader = DataLoader(t_dataset, batch_size=args.batch_size, shuffle=False, num_workers=8)
print(len(t_dataset))
...
if __name__ == '__main__':
    ...
    if args.eval:
        ...
        for i_batch, sample_batched in bar(enumerate(test_dataloader, 0)):
            ...
            tactile = sample_batched[0].clone().detach().to(device).float()
            heatmap = sample_batched[1].clone().detach().to(device).float()
            keypoint = sample_batched[2].clone().detach().to(device).float()
            tactile_frame = sample_batched[3].clone().detach().to(device).float()
            ...
            # Assign NumPy arrays to the lists
            y_true_batch = keypoint.cpu().detach().numpy().reshape(-1)
  
            threshold = 0.5
          
            # Convert y_true_batch to binary format using the same threshold
            y_true_binary_batch = (y_true_batch > threshold).astype(int)
            y_pred_binary_batch = (keypoint_out.cpu().detach().numpy() > threshold).astype(int).reshape(-1)
        ...
        # Calculate evaluation metrics for all batches
        accuracy = accuracy_score(y_true_binary_batch, y_pred_binary_batch)
        accuracy_list.append(accuracy)  # Append accuracy to the list
        batch_numbers.append(i_batch)  # Append batch number to the list
        precision = precision_score(y_true_binary_batch, y_pred_binary_batch, average='weighted')
        recall = recall_score(y_true_binary_batch, y_pred_binary_batch, average='weighted')
        f1 = f1_score(y_true_binary_batch, y_pred_binary_batch, average='weighted')
        conf_matrix = confusion_matrix(y_true_binary_batch, y_pred_binary_batch)
    ...
    print("Confusion Matrix for Batch", i_batch, ":\n", conf_matrix)
    ...
    for epoch in range(args.epoch):
        ...
        for i_batch, sample_batched in bar(enumerate(train_dataloader, 0)):
            ...
            heatmap = sample_batched[1].clone().detach().to(device).float()
            keypoint = sample_batched[2].clone().detach().to(device).float()
            idx = sample_batched[3].clone().detach().to(device).float()
...
#
#
# Load the saved training and validation loss data
loss_data = pickle.load(open(args.exp_dir + 'log/' + args.exp +
                              '_' + str(args.lr) + '_' + str(args.window) + '.p', "rb"))

train_loss_list = loss_data[0]
val_loss_list = loss_data[1]

# Assuming you have recorded loss for 25 epochs
epochs = range(1, 26)

# Plot the training and validation loss
plt.figure(figsize=(10, 6))
plt.plot(epochs, train_loss_list[:25], label='Training Loss')
